...
func (qb *GalleryStore) setGallerySort(query *queryBuilder, findFilter *models.FindFilterType) error {
	...
	if findFilter == nil || findFilter.Q == nil || *findFilter.Q == "" {
		...
		query.sortAndPagination += " ORDER BY galleries.pinned DESC"

		// Handle pinned sorting for non-search queries
		if sort == "play_count" {
			query.sortAndPagination += getCountSortWithoutOrderBy(galleryTable, galleriesViewDatesTable, galleryIDColumn, direction)
			...
			addFileTable()
			addFolderTable()
			query.sortAndPagination += fmt.Sprintf(", COALESCE(folders.path, '') || COALESCE(file_folder.path, '') || COALESCE(files.basename, '') COLLATE NATURAL_CI %s", direction)
		} else {
			query.sortAndPagination += getSortWithoutOrderBy(sort, direction, "galleries")
		}
