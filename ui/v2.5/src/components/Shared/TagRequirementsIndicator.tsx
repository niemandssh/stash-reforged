...
export const TagRequirementsIndicator: React.FC<
  ITagRequirementsIndicatorProps
> = ({ tags, colorPresets }) => {
  const intl = useIntl();
  ...
  const presetsWithRequirements = colorPresets.filter(
    (preset) =>
      ...
      preset.tag_requirements_description.trim() !== ""
  );

  // Separate required and optional presets
  const requiredPresets = presetsWithRequirements.filter(
    (preset) => preset.required_for_requirements ?? true
  );
  const optionalPresets = presetsWithRequirements.filter(
    (preset) => !preset.required_for_requirements
  );

  // Calculate max chip width and apply to all chips
  useEffect(() => {
    if (listRef.current) {
      const chips = listRef.current.querySelectorAll(
        ".tag-preset-name-block span"
      );
      let maxWidth = 0;
  ...
  const isFullySatisfied =
    ...
    filledRequiredCount === totalRequiredCategories;
