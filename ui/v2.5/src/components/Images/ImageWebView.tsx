...
export const ImageWebView: React.FC<IImageWebViewProps> = ({
  ...
  const handleImageLoad = useCallback(
    (imageId: string, naturalWidth: number, naturalHeight: number) => {
      setImageSizes((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    const calculateInterfaceHeight = () => {
      ...
      if (webControls)
        totalHeight += webControls.getBoundingClientRect().height;
    ...
    (image: GQL.SlimImageDataFragment) => {
      if (webDisplayMode === WebDisplayMode.FitToScreen) {
        ...
        if (imageSize) {
          ...
          if (
            imageSize.height <= availableHeight &&
        ...
        {images.map((image, index) => (
          <React.Fragment key={image.id}>
            ...
            >
              ...
              />
              {image.title && (
                <div className="image-web-title">{image.title}</div>
              )}
            </div>
          </React.Fragment>
        ))}
