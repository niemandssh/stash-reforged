...
export const ScenePlayer: React.FC<IScenePlayerProps> = PatchComponent(
  ...
  ({
    ...
    const nextScene = useNextScene(
      scene.id,
      similarScenesData?.findScene?.similar_scenes,
      Array.from(viewedScenes as Set<string>)
    );
    ...
    const updateVideoJsProgressBarTrimStyles = useCallback(
      (player: VideoJsPlayer) => {
        ...
        const existingOverlays =
          progressHolder.querySelectorAll(".vjs-trim-overlay");
    ...
    useEffect(() => {
      if (
        !posterFilterIdRef.current ||
        posterFilterSceneIdRef.current !== scene.id
      ) {
        posterFilterIdRef.current = `scene-player-poster-${
          scene.id
      ...
      const applyFiltersToPoster = () => {
        ...
        if (svgFilterId && scene.video_filters) {
          ...
          if (needsMatrix || needsGamma) {
            ...
            if (needsMatrix) {
              const whiteBalance = filters.white_balance ?? 100;
