...
function getItems(result: GQL.FindSceneMarkersQueryResult) {
  return (result as any)?.data?.findSceneMarkers?.scene_markers ?? [];
}

function getCount(result: GQL.FindSceneMarkersQueryResult) {
  return (result as any)?.data?.findSceneMarkers?.count ?? 0;
}
...
export const SceneMarkerList: React.FC<ISceneMarkerList> = ({
  ...
  async function playRandom(
    ...
    if (result.data?.findSceneMarkers) {
      ...
      if ((singleResult.data as any).findSceneMarkers.scene_markers.length === 1) {
        // navigate to the scene player page
        const url = NavUtils.makeSceneMarkerUrl(
          (singleResult.data as any).findSceneMarkers.scene_markers[0]
        );
        history.push(url);
      }
    }
