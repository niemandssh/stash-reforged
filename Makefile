...
.PHONY: ui-start
ui-start: ui-env
	cd ui/v2.5 && bun run start --host

.PHONY: fmt-ui
fmt-ui:
	cd ui/v2.5 && bun run format

# runs all of the frontend PR-acceptance steps
.PHONY: validate-ui
validate-ui:
	cd ui/v2.5 && bun run validate

# these targets run the same steps as fmt-ui and validate-ui, but only on files that have changed
fmt-ui-quick:
	cd ui/v2.5 && bun run prettier --write $$(git diff --name-only --relative --diff-filter d . ../../graphql)
...
validate-ui-quick:
	...
	if [ -n "$$tsfiles" ]; then bun run eslint $$tsfiles; fi && \
	if [ -n "$$scssfiles" ]; then bun run stylelint $$scssfiles; fi && \
	if [ -n "$$prettyfiles" ]; then bun run prettier --check $$prettyfiles; fi
